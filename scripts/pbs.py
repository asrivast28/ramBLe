...
def parse_args():
    ...
    parser.add_argument('-p', '--procs', type=int, default=cpu_count(), metavar='PROCS', help='Number of processes per node.')
    parser.add_argument('-A', '--account', type=str, metavar='NAME', help='Name of the account.')
    parser.add_argument('-q', '--queue', type=str, metavar='NAME', help='Name of the queue.')
    ...
    if 'hive' in gethostname():
        # Hive specific defaults
        if args.queue is None:
            args.queue = 'hive'
    if 'phoenix' in gethostname():
        # Phoenix specific defaults
        if args.account is None:
            args.account = 'GT-saluru8-CODA20'
        if args.queue is None:
            args.queue = 'inferno'
    if args.queue is None and args.account is None:
        raise argparse.ArgumentTypeError('Could not determine queue and/or account information for the job')
    return args
